...
try {
	if (os.platform() === 'win32') {
		console.warn('Warning: Timekit does not output correct time on Windows -- It will not be used');
	}else{
		timekit = require('timekit');
	}
}catch(exp){
	console.warn('Timekit Module is Not Installed');
}
...
if (os.platform() === 'solaris' || os.platform() === 'linux' || os.platform() === 'sunos' || os.platform() === 'darwin' || os.platform() === 'win32') {
	exports = module.exports = global.nodefly = new Nodefly();
}
else {
	exports = module.exports = { profile: function(){
		console.error('SORRY: the nodefly agent does not yet work on your platform(%s).', os.platform());
	}};
}
